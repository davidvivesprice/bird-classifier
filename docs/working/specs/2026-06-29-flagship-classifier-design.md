...
**Date:** 2026-06-29
> **STATUS NOTE (2026-09-18, from the 2026-09-17 post-mortem — [`../specs/2026-09-17-flagship-postmortem.md`](../specs/2026-09-17-flagship-postmortem.md)):** Phase 1 is **NOT done**. The pool this dossier calls "weak AIY labels" is not: **71 % of the manifest's 114,717 train rows (81,176) are labelled by the 12-class yard model** — the iMac's lock-time vote has been the canonical `common_name` (and the folder an image is filed under) since RC3 on 2026-04-25 — and on **43 % of train rows (49,894) AIY disagrees with the label**. Contact sheets put sampled label precision near 0.1–0.2 for the yard-labelled Goldfinch / Carolina Wren / Junco pools. The manifest also carries no camera column (12,257 ground-camera train rows), its test split drops the 231 `wrong→species` corrections, and all 138 `reclassify` rows have an empty `correct_species`. Nothing named "flagship" has ever been trained. Rebuild the manifest against the post-mortem's §3 must-fix list (provenance columns, `crop_valid` eras, contact-sheet gate, visit-grouped + chronological splits) before any training.
...
- **246,725 classifications** (weak AIY labels — **correction 2026-09-18: since 2026-04-25 the iMac's canonical label is the yard model's lock-time vote, not AIY's; see the status note above**) — the training pool. Visitor distribution (top, by count): House Finch 38.6k, Black-capped Chickadee 17.2k, Dark-eyed Junco 14.7k, White-breasted Nuthatch 10.4k, Mourning Dove 8.5k, Song Sparrow 7.4k, Downy Woodpecker 6.7k, American Goldfinch 5.7k, Carolina Wren 5.6k, Northern Cardinal 5.0k, Tufted Titmouse 4.7k, then Cowbird/Hairy/House Sparrow/Grackle/Mockingbird/Robin/Crow/White-throated Sparrow (1–2k each). (102.6k rows are blank/unlabeled.)
- **2,285 human reviews = GROUND TRUTH** (never train on these): 1,353 `correct`, 138 `reclassify` (~~corrected label~~ — all 138 have an empty `correct_species`, post-mortem 2026-09-17), 388 `wrong` (hard negatives), 339 `trash` (non-bird/empty → **OOD negatives**), plus skip/requeued.
