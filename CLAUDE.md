...
>
> The iMac-side repo at `/Users/vives/bird-classifier/` is the iMac deployment's repo — both repos are worked by the same operator now, but still don't push to it from here.
>
> The two-Claude patch workflow (`docs/historical/progress/cross-claude-comms.md`) is historical — one operator owns both repos now and cross-cutting fixes are cherry-picked directly.
>
> Both repos share git history through commit `5773551` (split point, 2026-04-25). They diverge from there.
>
> See `docs/historical/2026-04-25-pi-repo-split.md` for the full split context.
> The Pi reference book (chapters 00-12) lives at `~/docs/bird-observatory-pi/`, parallel to iMac's at `~/docs/bird-observatory/`.
>
> **Remote branch model (added 2026-05-11):**
> - iMac repo pushes to `imac-origin/main` (long-lived).
> - **Pi repo (this one) pushes to `imac-origin/pi-main`** (long-lived). The local branch is `pi-main`, configured to track it (`git status` should show `pi-main...imac-origin/pi-main`).
> - Ad-hoc snapshot branches for outside review (`pi-snapshot-YYYY-MM-DD`, `pi-overnight-YYYY-MM-DD`) are fine; delete when done.
> - **Do NOT push the Pi-repo's `pi-main` to `imac-origin/main`** — that's the iMac repo's branch and the histories have diverged since the April 25 split.
...
SQLite is the sole data store: `~/bird-snapshots/logs/classifications.db` (per-classification rows), `pipeline.db` (event store), `pi_reviews.db` (Pi-native ✓/✗ verdicts), plus `~/bird-snapshots/birdnet-audio/birdnet_local.db` (audio detections).

For the full reference, see the chapter book at `~/docs/bird-observatory-pi/` (00-overview through 12-audio).

### Services (5 long-running systemd-user units + hardening timers — units in `deploy/systemd/`, thermal watcher in `tools/`)
...
| `bird-audio.service` | — | BirdNET V2.4 on the feeder mic → `birdnet_local.db` (self-healing RTSP, ported 2026-07-07) |
...
go2rtc (RTSP) → **supervised decode+detect CHILD PROCESS** (`pipeline/frame_capture_proc.py`, 2026-07-03: PyAV decode of `feeder-sub` 640×360 + HailoDetector YOLOv8s **full-frame — every frame while birds are detected, every `PIPELINE_IDLE_STRIDE`(=2)-th frame when idle** — shipped through a shared-memory ring — both native SEGV crashers, libav and libhailort, are caged there; the parent survives their crashes and the tracker coasts through the ~2-4 s respawn; `PIPELINE_DECODE_INPROC=1` reverts) → **BirdTrackerV4** (2026-09-17, `pipeline/tracker_v4.py`, in the parent: Kalman box filter gated on the PREDICTED box, ByteTrack two-stage association with a 0.15–0.30 rescue band, 1.5 s visible coast → 10 s hidden lost buffer with colour + size + classifier-vote re-identification; detector child threshold `PIPELINE_DET_CONF`=0.15 feeds the rescue band; `PIPELINE_TRACKER=v3` selects the old Norfair tracker for A/B; measured on the may10 reel 26→9 ids for 9 visits, per-visit correct species 2/9→6/9) → PiClassifier (AIY ONNX on CPU, ~7.4 ms / crop) → vote lock → SnapshotWriter (1080p frame extracted from the HLS passthrough-mux segments by PTS at lock; AIY rerun) → SSE broadcast → dashboard. See `docs/working/progress/2026-07-03-native-crash-isolation.md`.

> **2026-06-29 foundations changes:** MotionGate/MOG2 is **bypassed on the Hailo path** — HailoDetector runs full-frame and ignores motion regions, so computing MOG2 every frame was wasted CPU and the thermal hog (F1). Tracker coast tuning (`hit_counter_max` 150, `distance_threshold` 2.5, `PIPELINE_TRACK_*`) applied to the v3 Norfair tracker only and is **superseded by BirdTrackerV4 (2026-09-17)** — v3 stays selectable for A/B. Classifier vote-eligibility floor 0.25→**0.16** (`PIPELINE_CLASSIFIER_FLOOR`), `MAX_CLASSIFICATION_ATTEMPTS` 5→12 (F2); lock gate now **calibrated** (≥3 votes, ≥**0.70 calibrated P(correct)** via `pipeline/calibration.py` isotonic map, ≥60% agreement; env `PIPELINE_LOCK_CONF`). The hi-res ring buffer is legacy (None); snapshots come from the HLS-by-PTS path. See `docs/historical/specs/2026-06-27-pi-live-id-foundations-design.md`.

Per-camera classifier config: only feeder camera enabled (ground commented out in `bird_pipeline_v3.py:CAMERAS_DETECT`). Pi has no Coral; AIY runs on CPU. Hailo lab models (ResNet50 classifier, YOLOv6n detector candidate) cohabit with the YOLOv8s detector on a single shared `VDevice` via the HailoRT scheduler — see `~/docs/bird-observatory-pi/04-hailo-engine.md` and `docs/working/specs/2026-04-25-hailo-playbook.md`.

### Video Path

- **Local**: Browser → `<video-stream>` element → WebRTC (signaled via the dashboard's `/api/ws` go2rtc proxy; media UDP, real-time, sub-second)
- **Remote**: Browser → MSE via the same `/api/ws` proxy on wss://pi5.vivessato.com (TCP, buffered, auto-fallback)
