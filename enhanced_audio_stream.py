...
Captures RTSP audio via python-av, applies bandpass + noisereduce + RMS
normalization, encodes to MP3 via ffmpeg, and serves as a streaming HTTP
endpoint. The dashboard toggles between this and the raw camera feed.

Architecture:
  RTSP → python-av (decode) → preprocess → ring buffer → ffmpeg (encode) → HTTP

A single background thread decodes the RTSP stream and fills a shared ring
buffer with processed PCM. Each HTTP client spawns an ffmpeg encoder that
reads from the buffer and streams MP3 to the browser.
"""
...
# ── Audio preprocessing (same as audio_analyzer.py) ────────────────────────
BANDPASS_LOW = 300
BANDPASS_HIGH = 15000
BANDPASS_ORDER = 4
...
def preprocess_chunk(audio_float32):
    """Apply bandpass + noisereduce + RMS match."""
...
def main():
    ...
    log.info("  Noise reduction: %s", "ON" if NOISE_REDUCE_ENABLED else "OFF")
